...
class CbrAppFinanceManager extends CI_Controller
{
    private $Date;
    private $DateTime;
    ...
    {
        $this->data['page_title'] = "Finance Manager Approval-Cash Book Requisition";
        $this->data['page_content'] = "cbr_app/finance_manager";
        $this->data['script_page'] =  '<script src="' . base_url() . 'assets/Pages/cbr_app/finance_manager.js"></script>';
    ...
    public function approve_submission()
    {
        ...
        foreach ($Cbrs as $CBReq_No) {
            $this->db->where('CBReq_No', $CBReq_No)->update($this->Ttrx_Cbr_Approval, [
                'Status_AppvFinanceManager' => 1,
                'AppvFinanceManager_Name' => $this->session->userdata('sys_sba_nama'),
                'AppvFinanceManager_By' => $this->session->userdata('sys_sba_username'),
                'AppvFinanceManager_At' => $this->DateTime,
            ]);
        }
    ...
    public function DT_List_To_Approve()
    {
        ...
        AND IsAppvFinanceManager = 1
        AND Status_AppvFinanceManager IS NULL
    ...
    public function DT_List_History_Approval()
    {
        ...
        AND Ttrx_Cbr_Approval.CBReq_No IS NOT NULL
        AND IsAppvFinanceManager = 1
        AND Status_AppvFinanceManager IS NOT NULL
        AND (IsAppvStaff = 0 or IsAppvStaff = 1 and Status_AppvStaff = 1)
        AND (IsAppvChief = 0 or IsAppvChief = 1 and Status_AppvChief = 1)
        AND (IsAppvAsstManager = 0 or IsAppvAsstManager = 1 and Status_AppvAsstManager = 1)
        AND (IsAppvManager = 0 or IsAppvManager = 1 and Status_AppvManager = 1)
