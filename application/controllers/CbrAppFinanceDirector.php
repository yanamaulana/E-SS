...
class CbrAppFinanceDirector extends CI_Controller
{
    ...
    public function check_attachment_existence()
    {
        ...
        // 1. Ambil data Attachment dari Database dengan JOIN ke ERPQview_User_Employee
        ...
        foreach ($attachments as $row) {
            // ... (Logika pengecekan file tetap sama) ...
        ...
        // 3. 🔥 OUTPUT TABEL HTML (Diperbarui untuk kolom baru)
        echo '<style>
        ...
        echo '<th>No.</th>';
        echo '<th>CBR No</th>';
        echo '<th>Nama Pembuat</th>'; // 🔥 KOLOM BARU
        ...
        if (empty($processed_attachments)) {
            echo '<tr><td colspan="6" style="text-align: center;">Tidak ada data attachment ditemukan di database.</td></tr>';
        } else {
            foreach ($processed_attachments as $data) {
                ...
                echo '<td>' . htmlspecialchars($data->Created_By_Name ?? '-') . '</td>'; // 🔥 Tampilkan Nama Pembuat
