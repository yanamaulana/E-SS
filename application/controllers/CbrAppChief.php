...
class CbrAppChief extends CI_Controller
{
    private $Date;
    private $DateTime;
    ...
    {
        $this->data['page_title'] = "Chief Approval-Cash Book Requisition";
        $this->data['page_content'] = "cbr_app/chief";
        $this->data['script_page'] =  '<script src="' . base_url() . 'assets/Pages/cbr_app/chief.js"></script>';
    ...
    public function approve_submission()
    {
        ...
        foreach ($Cbrs as $CBReq_No) {
            $this->db->where('CBReq_No', $CBReq_No)->update($this->Ttrx_Cbr_Approval, [
                'Status_AppvChief' => 1,
                'AppvChief_Name' => $this->session->userdata('sys_sba_nama'),
                'AppvChief_By' => $this->session->userdata('sys_sba_username'),
                'AppvChief_At' => $this->DateTime,
            ]);
        }
    ...
    public function DT_List_History_Approval()
    {
        ...
        AND Ttrx_Cbr_Approval.CBReq_No IS NOT NULL
        AND IsAppvChief = 1
        AND Status_AppvChief IS NOT NULL
        AND (IsAppvStaff = 0 or IsAppvStaff = 1 and Status_AppvStaff = 1)
        AND UserDivision IN ('" . $this->session->userdata('sys_cbr_divs') . "') ";
        // ORDER BY TAccCashBookReq_Header.Document_Date DESC,TAccCashBookReq_Header.CBReq_No DESC 
