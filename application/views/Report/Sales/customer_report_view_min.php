...
<body>
    ...
    $enddate = '2025-05-31';
    $languageList = ["SalesReport", "Print", "Close", "CustomerTransactionReport", "PrintedOn", "SONumber", "TotalAmount", "TotalDiscount", "TotalInvoiced", "SalesOrder", "CustomerRFQ", "RFQCode", "RFQDate", "NoTransactionAvailable", "ItemCode", "ItemName", "Qty", "Qty2", "NoRecordFound", "SODate", "QtyDelivered", "UnitPrice", "Amount", "Discount", "Tax", "CustomerName", "CustomerAddress", "Quotation", "SNDate", "IncludedPPN", "ProformaInvoice", "PINumber", "PIDate", "QuotationNumber", "QuotationDate", "SalesInvoice", "SINumber", "SIDate", "SalesReturn", "SRNumber", "SRDate", "ShipmentNote", "SNNumber", "UnitType", "UnitType2", "mthJanuary", "mthFebruary", "mthMarch", "mthApril", "mthMay", "mthJune", "mthJuly", "mthAugust", "mthSeptember", "mthOctober", "mthNovember", "mthDecember", "TotalAmount", "AdditionalDisc", "SalesContract", "SalesContractNumber", "SalesContractDate", "QuotationNumber", "QuotationDueDate", "SalesContractStartDate", "SalesContractEndDate", "DiscountValue", "category", "Dimension", "AccountName", "AccountCode", "Notes", "Debit", "Credit", "currency", "DirectSalesInvoice", "DocumentReference", "Type", "Color", "Brand", "InvoicePrintNo", "grandtotal", "ShippingInstructionNumber", "claimdeduction", "TotAmount1"];
    ...
    $qGetAccount = $this->db->query("SELECTDISTINCTAccount_ID,Account_Name,Account_Address1,AccountTitle_CodeFROMTACCOUNTWHERE (Cust_FG = 1)ANDCategory_ID IN (SELECT DISTINCT CATEGORY_ID FROM TDATAGROUPACCOUNT WHERE DATAGROUP_ID IN (20,86))AND Account_ID in ('12694','366')AND Company_ID = 2AND TACCOUNT.Flag <> 1ORDER BY Account_Name")->result_array();
    $customerrfq_gtqty = 0;
    $quotation_gtamt = 0;
    ...
    $totAmtDEUR = 0;
    $totAmtDUSD = 0;    ?>
    <?php foreach ($qGetAccount as $curr_row => $account) {
        $NoTransactionAvailable = 1;
        $query = "SELECT taccsi_header.invoice_number,taccsi_header.invoiceprintnumber,taccsi_header.invoice_date,taccsi_header.currency_id,taccsi_header.invoice_id,taccso_header.PO_NumCustomer,taccsi_detail.item_code,isnull(taccsi_detail.qty,0) as qty,isnull(taccsi_detail.unitprice,0) as unitprice,isnull(taccsi_detail.disc_percentage,0) as disc_percentage ,isnull(taccsi_detail.disc_value,0) as disc_value,taccsi_detail.tax_code1,taccsi_detail.tax_code2,titem.item_name,dimension_name,isnull(taccsi_header.transactiondiscountrate,0) as transactiondiscountrate, titem.customfield1 as item_type, titem.item_size as brand, tgscolor.color_name,taccsi_header.so_number,'claimdeduction' = case TAccSI_Header.Ori_Invoice_Amountwhen 0 then 0else (dbo.fnc_calcsalesreport(TAccSI_Detail.qty, TAccSI_Detail.unitprice, TAccSI_Header.Ori_Invoice_Amount, TAccSI_Header.claimdeduction, 1.000000000))end,'base_claimdeduction' = case TAccSI_Header.Ori_Base_Invoice_Amountwhen 0 then 0else (   dbo.fnc_calcsalesreport(TAccSI_Detail.qty, TAccSI_Detail.unitprice, TAccSI_Header.Ori_Invoice_Amount, TAccSI_Header.claimdeduction, TAccSI_Header.base_invoice_amount/TAccSI_Header.invoice_amount)) end,isnull(BOMDetail.BOM_TYPE,0) bom_type,isnull(prod_cost_percentage,0) prod_cost_percentage,isnull(loss_percentage,0) loss_percentage,isnull(total_cost,0) total_cost,isnull(loss_cost,0) loss_cost,isnull(salary2,0) salary2,isnull(prod_cost,0) prod_cost,isnull(TScale.scale,1) scale,isnull(BOMDetail.TotalCOGS * TScale.scale,0) as TotalCOGS,isnull(TBOM_Guitar.total_cost_rm , 0) as total_cost_rm,isnull(TBOM_Guitar_2.total_cost_rm2 , 0) as total_cost_rm2,isnull(TBOM_Guitar_3.total_cost_rm_3 , 0) as total_cost_rm_3from taccsi_headerinner join taccsi_detail WITH (NOLOCK) on taccsi_header.invoice_number = taccsi_detail.invoice_numberinner join titem WITH (NOLOCK) on taccsi_detail.item_code = titem.item_codeinner join titemdimension WITH (NOLOCK) on titemdimension.dimension_id = taccsi_detail.dimension_idleft  join taccso_header WITH (NOLOCK) on taccso_header.so_number = taccsi_detail.so_numberleft join tgscolor WITH (NOLOCK) on tgscolor.color_code = titem.item_colorLeft join (Select bom_type,item_code,   umr_salary, currency_id,loss_percentage, salary, prod_cost_percentage, sum(total_cost) TotalCost,sum(total_cost) total_cost,(((sum(total_cost)) * (loss_percentage)) / 100) loss_cost, salary salary2 ,((((sum(total_cost)) + (umr_salary)) * prod_cost_percentage) / 100) prod_cost,(sum(total_cost) + (((sum(total_cost)) * (loss_percentage)) / 100) + (salary) + ((((sum(total_cost)) + (umr_salary)) * prod_cost_percentage) / 100)) TotalCOGSfrom (selectbom_type,bom_code, item_code,  item_type, brand, umr_salary, currency_id,loss_percentage, salary, prod_cost_percentage,   sum(total_cost_converted) as total_cost from (selectbom_type,bom_code, item_code,  item_type, brand, umr_salary,  header_curr as currency_id,global_loss as loss_percentage, salary, prod_cost_percentage, cast((total_cost / isnull(scale, 1)) as money) as total_cost_converted from (selectdistinct  tppicitembom.bom_type,tppicitembom.bom_code, tppicitembom.item_code,  titem.customfield1 as item_type, titem.item_size as brand, tppicitembom.umr_salary, isnull(tppicitembom.currency_id, 'idr') as header_curr,isnull(tppicitembom.loss_percentage, 0) as global_loss, isnull(tppicitembom.salary, 0) as salary, isnull(tppicitembom.prod_cost_percentage, 0) as prod_cost_percentage,  tppicitembom_detail.rm_code, tppicitembom_detail.rm_qty, isnull(tppicitembom_detail.currency_id, 'idr') as details_curr, isnull(tppicitembom_detail.is_accessories, 0) as is_accessories, ( ((tppicitembom_detail.cost * tppicitembom_detail.item_convertion) * tppicitembom_detail.rm_qty) +((((tppicitembom_detail.cost * tppicitembom_detail.item_convertion) * tppicitembom_detail.rm_qty) * tppicitembom_detail.comp_loss_percentage) / 100)) as total_cost, ( select top 1 scale from tcurrencyconverter wherecurrency_id_1 = isnull(tppicitembom.currency_id, 'idr')andcurrency_id_2 = isnull(tppicitembom_detail.currency_id, 'idr')andtcurrencyconverter.status = 1order by last_update desc) as scale, ( case when tppicitembom_detail.is_expensive_parts = 1 then  tppicitembom_detail.loss_percentageelse 100end) as detail_loss_percentfrom tppicitembom_detailinner join tppicitembom on tppicitembom.bom_code = tppicitembom_detail.bom_codeinner join  (select item_code,MAX(LAST_UPDATE) LAST_UPDATEfrom TPPICITEMBOM GROUP BY item_code) last_TPPICITEMBOM ON   (TPPICITEMBOM.item_code=last_TPPICITEMBOM.item_code) AND (TPPICITEMBOM.LAST_UPDATE=last_TPPICITEMBOM.LAST_UPDATE)inner join titem on titem.item_code = tppicitembom.item_code  where  1=1) first_layerwhereisnull(first_layer.total_cost, 0) <> 0 and first_layer.total_cost <> 0) second_layergroup bybom_type,bom_code, item_code,  item_type, brand,  umr_salary, currency_id, loss_percentage, salary, prod_cost_percentage, total_cost_converted ) T    Group by bom_type,bom_code, item_code,  item_type, brand, umr_salary, currency_id,loss_percentage, salary,prod_cost_percentage) BOMDetail on (taccsi_detail.item_code=BOMDetail.item_code)Left Join vw_TBOM_Guitar AS TBOM_Guitar WITH (NOLOCK)  ON (taccsi_detail.item_code=TBOM_Guitar.item_code)Left Join vw_TBOM_Guitar_2 AS TBOM_Guitar_2 WITH (NOLOCK) ON (taccsi_detail.item_code=TBOM_Guitar_2.item_code)Left join vw_TBOM_Guitar_3 AS TBOM_Guitar_3 WITH (NOLOCK) ON (taccsi_detail.item_code=TBOM_Guitar_3.item_code)Left Join (selectdistinct x.currency_id_1 as curr_to, x.currency_id_2 as curr_from, (select top 1 scale from TCurrencyConverter where currency_id_1 = x.currency_id_1 and currency_id_2 = x.currency_id_2 order by last_update desc) as scale from (selectdistinct tcurrencyconverter.currency_id_1, currency_id_2from tcurrency inner join tcurrencyconverteron tcurrency.currency_id = tcurrencyconverter.currency_id_1where tcurrency.status = 1) x) TScale on (TScale.curr_from=BOMDetail.currency_id) and (TScale.curr_to=taccsi_header.currency_ID) where taccsi_header.account_id = '" . $account['Account_ID'] . "'and taccsi_header.invoice_date >= {d '$startdate'}and taccsi_header.invoice_date <= {d '$enddate'}and isnull(taccsi_header.isvoid,0) = 0order by taccsi_header.invoice_number";

        $sql = "SELECT  taccsi_header.invoice_number,taccsi_header.invoiceprintnumber,taccsi_header.so_number,taccsi_header.invoice_date,taccsi_header.directtype,taccsi_header.currency_id ,taccchartaccount.account_number,taccchartaccount.account_nameen as acc_name,taccjournaldetail.notes,journald_debet, journald_kredit,taccsi_header.so_number,0 as claimdeduction,0 as base_claimdeductionfromtaccsi_headerinner join taccjournaldetail on taccjournaldetail.journalh_code = taccsi_header.invoice_numberinner join taccchartaccount ontaccjournaldetail.acc_id = taccchartaccount.acc_id where   taccjournaldetail.isextra = 1and taccsi_header.isdirect = 1and taccsi_header.account_id = '" . $account['Account_ID'] . "'andtaccsi_header.invoice_date >= {d '$startdate'}andtaccsi_header.invoice_date <= {d '$enddate'}and isnull(taccsi_header.isvoid,0) = 0order by taccsi_header.invoice_number";
        ...
        foreach ($qSelectTransactionSalesInvoice as $index => $row) {
            ...
            $qSalesInvoicecategoryitem = $this->db->query("SELECT itemcategory_name             FROM titemcategory            INNER JOIN titemcompany ON titemcompany.itemcategory_id = titemcategory.itemcategory_id            WHERE item_code = '" . $row['item_code'] . "'");
            ...
            if ($row['bom_type'] > 3 || $row['bom_type'] == 0) {
                ...
                echo "<td class='formtextreport' align='right'>" .                    number_format((round($total_COGS, 2) + round($total_lossCOGS, 2)) / $row['scale'], 2, '.', ',') .                    "</td>";
                echo "<td class='formtextreport' align='right'>" .                    number_format(round($row['salary2'], 2) / $row['scale'], 2, '.', ',') .                    "</td>";
                ...
                echo "<td class='formtextreport' align='right'>" .                    number_format(($total_cog / $row['scale']) -                            ($total_prod_cost / $row['scale']) -                            (round($row['salary2'], 2) / $row['scale']),                        2,                        '.',                        ',') .                    "</td>";
                echo "<td class='formtextreport' align='right'>" .                    number_format(round($row['salary2'], 2) / $row['scale'], 2, '.', ',') .                    "</td>";
                echo "<td class='formtextreport' align='right'>";
